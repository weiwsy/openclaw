...
export function resetChatStateForRouteSession(
  ...
  state.lastChatStreamMediaUrls = null;
...
export function createPageState(
  ...
  const state = {
    ...
    chatStreamMediaUrls: null as string[] | null,
    lastChatStreamMediaUrls: null as Record<string, string[] | undefined> | null,
    lastError: null,
    chatError: null,
    chatRunError: null,
    agentsError: null,
...
export function handlePageGatewayEvent(state: ChatPageHost, event: GatewayEventFrame) {
  ...
  if (event.event === "agent" || event.event === "session.tool") {
    // Capture mediaUrls from assistant stream events for injection into the final chat message.
    const agentPayload = event.payload as { stream?: string; data?: { mediaUrls?: string[]; text?: string }; runId?: string } | undefined;
    if (agentPayload?.stream === "assistant" && agentPayload?.data?.mediaUrls?.length) {
      state.chatStreamMediaUrls = agentPayload.data.mediaUrls;
      // Also key by runId as fallback when the final event comes after chatStream reset.
      if (typeof agentPayload.runId === "string" && agentPayload.runId.trim()) {
        state.lastChatStreamMediaUrls = {
          ...(state.lastChatStreamMediaUrls ?? {}),
          [agentPayload.runId]: agentPayload.data.mediaUrls,
        };
      }
    }
