...
function handleChatEvent(state: ChatState, payload?: ChatEventPayload) {
  ...
  if (payload.state === "delta") {
    ...
    // Inject captured mediaUrls into the final message so splitMediaFromOutput
    // normalizes them into attachment blocks. The server strips MEDIA: lines
    // from the agent text and sends them via the agent event's mediaUrls field,
    // but the UI ignores that field during streaming. Re-insert them here so
    // the normal message normalization pipeline handles attachments correctly.
    const runId = typeof payload.runId === "string" ? payload.runId : undefined;
    const mediaUrls = state.chatStreamMediaUrls?.length
      ? state.chatStreamMediaUrls
      : runId && state.lastChatStreamMediaUrls?.[runId]?.length
        ? state.lastChatStreamMediaUrls[runId]
        : undefined;
    if (mediaUrls?.length) {
      const textBlock = Array.isArray(payload.message?.content)
        ? (payload.message.content as Array<Record<string, unknown>>).find(
      ...
      if (textBlock && typeof textBlock.text === "string") {
        ...
        // Handle flat-text format (no content array)
        payload.message.text = `${payload.message.text}\n\n${mediaUrls.map((url) => `MEDIA:${url}`).join("\n")}`;
      }
      state.chatStreamMediaUrls = null;
      if (runId && state.lastChatStreamMediaUrls) {
        const next = { ...state.lastChatStreamMediaUrls };
        delete next[runId];
        state.lastChatStreamMediaUrls = Object.keys(next).length > 0 ? next : null;
      }
    }
    const finalMessage = normalizeFinalAssistantMessage(payload.message);
